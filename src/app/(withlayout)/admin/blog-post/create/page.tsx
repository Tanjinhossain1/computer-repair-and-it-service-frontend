  ...
  return (
    <Form style={{margin: "30px"}} submitHandler={onSubmit}>
      <Row gutter={16}>
        ...
        <Col xs={24} sm={12}>
          <FormInput placeholder="short description" label="Top Short Description" name="topShortDescription" />
        </Col>
        <Col xs={24} sm={12}>
          <FormTextArea rows={5} placeholder="description" label="Description" name="description" />
        </Col>
        <Col style={{marginTop:"20px"}} xs={24} sm={12}>
          <UploadImageField runAfterChange={(file)=>setUnFormatFile(file)} name="image" />
        </Col>
      </Row>
      <Button style={{marginTop:"20px",textAlign:"center"}} type="primary" htmlType="submit">
        Submit
      </Button>
    </Form>
  );
